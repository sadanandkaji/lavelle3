        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#fcfcf9] relative no-scrollbar" style={{ backgroundImage: 'url("https://www.transparenttextures.com/patterns/natural-paper.png")' }}>
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.type === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`p-4 rounded-[1.5rem] max-w-[88%] sm:max-w-[85%] text-[14px] leading-relaxed shadow-sm border ${msg.type === "user" ? "bg-[#2d6a4f] text-white rounded-tr-none border-transparent" : "bg-white text-gray-800 rounded-tl-none border-stone-200"}`}>
                {msg.text.split("\n").map((line, j) => <div key={j} className="mb-1 last:mb-0">{formatMessage(line)}</div>)}
              </div>
            </div>
          ))}
          {loading && (
        ...
        {availableTemplates.length > 0 && (
          <div className="px-4 py-3 bg-white border-t border-stone-100 flex overflow-x-auto no-scrollbar gap-2 shrink-0">
            {availableTemplates.map(t => (
              <button key={t.id} onClick={() => sendMessage(t.id, t.query)} className="whitespace-nowrap text-[12px] font-semibold text-stone-700 bg-stone-50 hover:bg-[#2d6a4f] hover:text-white px-5 py-2 rounded-full border border-stone-200 transition-all active:scale-95 shadow-sm">
                {t.label}
              </button>
            ))}
          </div>
